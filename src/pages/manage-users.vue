<template>
  <div class="bg-gray-200 w-full h-full font-inter">
    ...
    <h1 class="font-inter text-3xl">
    Accounts
  </h1>
    <button class="bg-[#9B111E] text-white px-3 py-2 rounded shadow-lg 
    transition-transform duration-300 transform hover:scale-105 font-semibold font-inter"
  ...
  <div class="flex flex-row items-center m-5 space-x-5">
    <div class="flex justify-start w-52 h-20 bg-white rounded-lg shadow-lg px-2 items-center border-l-2 border-green-400 space-x-5">
        <h2 class="font-inter text-4xl font-bold mb-0 ml-5">{{ totalSuppliers }} <span class = "text-sm antialiased text-gray-600 ml-2">suppliers</span></h2>
    </div>
    <div class="flex justify-start w-52 h-20 bg-white rounded-lg shadow-lg px-2 items-center border-l-2 border-blue-400 space-x-5">
        <h2 class="font-inter text-4xl font-bold mb-0 ml-5">{{ totalStaff }} <span class = "text-sm antialiased text-gray-600 ml-2">staff</span></h2>
    </div>
    <div class="flex justify-start w-52 h-20 bg-white rounded-lg shadow-lg px-2 items-center border-l-2 border-blue-400 space-x-5">
        <h2 class="font-inter text-4xl font-bold mb-0 ml-5">{{ totalAdmin }} <span class = "text-sm antialiased text-gray-600 ml-2">admin</span></h2>
    </div>
...
<div class="flex flex-row justify-between items-center m-5 space-x-5">
  <div class = "flex">
  <button :class="[ 
    'flex justify-center items-center w-28 h-10 m-2 font-inter font-semibold rounded-lg shadow-lg transition-transform duration-300 transform hover:scale-105', 
...
<button class = "mr-2 w-32 h-10 bg-[#9B111E] font-semibold text-gray-100 font-inter rounded-md shadow-lg 
...
<!--- Supplier Table --->
<div v-if="showTable === 'Suppliers'" class="relative shadow-md sm:rounded-xl w-full max-w-[1170px] h-[200] ml-5 mt-2 font-inter mb-10">
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 mb-4 max-h-30 table-fixed">
            ...
            <tbody>
                <tr
                    ...
                    <th scope="row" class="w-16 px-2 py-3 font-inter text-gray-900 whitespace-nowrap dark:text-white">{{ supplier.dummyIndex }}</th>
  ...
  <!--- Staff Table--->
  <div v-if="showTable === 'Staffs'" class="relative shadow-md sm:rounded-xl w-full max-w-[1170px] h-[200] ml-5 mt-2 font-inter mb-10">
    <div class="overflow-x-auto">
      <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 mb-4 max-h-30 table-fixed">
      <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
        <tr>
  ...
  <!-- Admin Table -->
<div v-if="showTable === 'Admin'" class="relative shadow-md sm:rounded-xl w-full max-w-[1170px] h-[200] ml-5 mt-2 font-inter mb-10">
    <div class="overflow-x-auto">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 mb-4 max-h-30 table-fixed">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
    ...
    <div class="m-5">
      ...
      <div class="flex flex-row">
        <div class="flex flex-col space-y-1 w-full">
          ...
          <input type="text" class="p-2 h-10 rounded-lg text-sm shadow-md border border-gray-500 focus:outline-none focus:border-blue-700" v-model="firstName" required>
        </div>
        <div class="flex flex-col space-y-1 w-full ml-2">
          <label class="text-xs text-gray-600 ml-2 text-start">Last Name</label>
          <input type="text" class="p-2 h-10 rounded-lg text-sm shadow-md border border-gray-500 focus:outline-none focus:border-blue-700" v-model="lastName" required>
        </div>
      </div>
      ...
      <div class="flex flex-col mt-5">
        ...
        <input type="text" class="mt-1 p-2 w-full h-10 rounded-lg text-sm shadow-md border border-gray-500 focus:outline-none focus:border-blue-700" v-model="username" @click="deriveUsername" required>
      </div>

      <!-- Email -->
      <div class="flex flex-col  mt-5">
        <label class="text-xs text-gray-600 ml-2 text-start">Email</label>
        <input type="text" class="mt-1 p-2 w-full h-10 rounded-lg text-sm shadow-md border border-gray-500 focus:outline-none focus:border-blue-700" v-model="email" @click="deriveEmail" required>
      </div>

      <!-- Contact Number -->
      <div class="flex flex-col  mt-5">
        <label class="text-xs text-gray-600 ml-2 text-start">Contact Number</label>
        <input type="text" class="mt-1 p-2 w-full h-10 rounded-lg text-sm shadow-md border border-gray-500 focus:outline-none focus:border-blue-700" v-model="contactNumber" required>
      </div>

      <!-- Address -->
      <div class="flex flex-col  mt-5">
        <label class="text-xs text-gray-600 ml-2 text-start">Address</label>
        <input type="text" class="mt-1 p-2 w-full h-10 rounded-lg text-sm shadow-md border border-gray-500 focus:outline-none focus:border-blue-700" v-model="address" required>
      </div>

      <!-- User Type Selection -->
      <div class="flex flex-col  mt-5">
        <label class="text-xs text-gray-600 ml-2 text-start">Account Type</label>
        <select class="flex mt-1 p-2 w-full h-10 rounded-lg text-sm shadow-md border border-gray-500 focus:outline-none focus:border-blue-700" v-model="selectedUserType" @change="selectAddForm">
      ...
      <div v-if="addSupplierDetails" class="mt-5">
        <select class="mt-2 p-2 w-full h-10 rounded-lg text-sm shadow-md border border-gray-500 focus:outline-none focus:border-blue-700" v-model="service">
...
<div v-if="showInactiveSuppliersModal" @click.self="closeInactiveSuppliersModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 overflow-y-auto flex justify-center items-center z-40">
  <div class="bg-white p-5 rounded-lg shadow-lg w-[1000px]">
    <div class="flex justify-between items-center mb-4">
      <h2 class="text-xl font-inter">Inactive Suppliers</h2>
...
<div v-if="showRateModal" @click.self="closeRateModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 overflow-y-auto flex justify-center items-center z-50">
  <div class="bg-white p-5 rounded-lg shadow-lg w-[400px]">
    <div class="flex justify-between items-center">
      <h2 class="text-xl font-inter">Set Supplier Rate</h2>
...
<div v-if="showSocialMediaModal" @click.self="closeSocialMediaModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 overflow-y-auto h-full w-full flex justify-center items-center z-40">
    <div class="bg-white p-8 rounded-lg shadow-xl w-[500px] relative">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-inter text-gray-800">Add Social Media</h2>
        ...
        <form @submit.prevent="submitSocialMedia" class="space-y-4">
            <div>
                <label class="block text-sm font-inter text-gray-700 mb-1 text-start">Platform *</label>
                <select 
            ...
            <div>
                <label class="block text-sm font-inter text-gray-700 mb-1 text-start">Handle/Username *</label>
                <input 
                    type="text" 
            ...
            <div>
                <label class="block text-sm font-inter text-gray-700 mb-1 text-start">URL (Optional)</label>
                <input 
                    type="url" 
...
<form v-if="editSupplierForm" class = "fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-30" @click.self="closeEditSupplierBtn">
  <div class = "bg-white w-[800px] p-5 rounded-lg shadow-lg  overflow-y-auto">
        <div class = "flex justify-between items-center m-3">
              <h1 class = "font-semibold text-xl font-inter text-gray-800">Update Supplier Info</h1>
        </div>
        <div class = "border border-gray-500 mt-5 items-center"></div>
          ...
          <div class="w-3/5 space-y-5">
            ...
            <div>
              ...
              <select v-model="selectedSupplier.service" class="mt-2 p-2 w-full h-10 rounded-lg shadow-md border border-gray-500 focus:outline-none focus:border-blue-700">
                ...
                <option value="Transportation">Transportation</option>
                <option value="Invitations">Invitations and Stationery</option>
                <option value="Keepsakes">Favors and Gifts</option>
              </select>
            </div>
          ...
          <div class="w-1/2">
            <div class="border border-gray-200 rounded-lg p-4">
              <div class="flex justify-between items-center mb-3">
                <p class="text-gray-700 font-inter">Social Media Accounts</p>
                  ...
                  <div v-for="social in selectedSupplier.socialMedia" :key="social.social_media_id" 
                    ...
                    <div class="flex-1">
                      <div class="flex items-center gap-2">
                        <span class="font-inter text-gray-700">{{ social.platform }}</span>
            ...
            <!-- Add Set Rate button here -->
            <div class="mt-4">
              <button type="button"
                      @click.prevent.stop="showRateModal = true" 
...
<form v-if="editStaffForm" class = "flex justify-center items-center fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-20" @click.self="closeEditStaffBtn">
  <div class = "bg-white w-[500px] p-5 rounded-lg shadow-lg  overflow-y-auto">
        <div class = "flex justify-between items-center m-3">
              <h1 class = "font-semibold text-xl font-inter text-gray-800">Update Staff Info</h1>
        </div>
        <div class = "border border-gray-500 mt-5 items-center"></div>
               <div class = "m-5">
...
<form v-if="editAdminForm" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-30" @click.self="closeEditAdminBtn">
  <div class="bg-white w-[500px] p-5 rounded-lg shadow-lg overflow-y-auto">
    <div class="flex justify-between items-center m-3">
      <h1 class="font-semibold text-xl font-inter text-gray-800">Update Admin Info</h1>
    </div>
    <div class="border border-gray-500 mt-5 items-center"></div>
    <div class="m-5">
  ...
  methods: {
    ...
    async handleRegister() {
            try {
                ...
                if (this.selectedUserType === 'Suppliers') {
                    if (!this.service) {
                        this.showErrorAlert('Vendor details are incomplete!');
                        return;
                    }
                }
        ...
        async fetchSuppliers() {
          try {
              const token = localStorage.getItem('access_token');
              if (!token) {
                  this.showErrorAlert('You are not logged in. Please log in to view supplier data.');
                  return;
              }
        ...
        async confirmEditStaff() {
            try {
              ...
              if (!this.selectedStaff || !this.selectedStaff.no) {
                this.showErrorAlert("Staff ID is missing or invalid.");
                return;
              }
              ...
              if (response.status === 200) {
                this.showSuccessAlert('Staff updated successfully!');
                ...
                this.showErrorAlert('Failed to update staff.');
              }
            } catch (error) {
              console.error('Error updating staff:', error);
              if (error.response) {
                this.showErrorAlert(`Error updating staff: ${error.response.data.message}`);
              } else {
                this.showErrorAlert('Error updating staff. Please try again.');
              }
            }
          },
          ...
          async confirmEditSupplier() {
              try {
                ...
                if (!this.selectedSupplier) {
                  this.showErrorAlert("No supplier selected for editing.");
                  return;
                }

                if (!this.selectedSupplier.no) {
                  this.showErrorAlert("Supplier ID is missing or invalid.");
                  return;
                }
                ...
                if (!userid) {
                  this.showErrorAlert("User ID is missing for this supplier. Unable to edit.");
                  return;
                }

                const requiredFields = [
                  'fullName', 'email', 'contact', 'username', 'password'
                ];
                ...
                if (missingFields.length > 0) {
                  ...
                  this.showErrorAlert(`Missing fields: ${missingFields.join(', ')}`);
                  return;
                }

                const [firstname, ...lastnameParts] = this.selectedSupplier.fullName.split(' ');
                if (!firstname || lastnameParts.length === 0) {
                  this.showErrorAlert("Full name must contain both first and last name.");
                  return;
                }
                const lastname = lastnameParts.join(' ');
                ...
                if (response.status === 200) {
                  this.showSuccessAlert("Supplier updated successfully!");
                ...
                if (error.response) {
                  ...
                  this.showErrorAlert("Error updating supplier. Please try again.");
                }
              }
            },
          ...
          async deleteUserItem(userid) {
              try {
                ...
                const token = localStorage.getItem('access_token');
                if (!token) {
                  this.showErrorAlert('You are not logged in. Please log in to delete users.');
                  return;
                }
                ...
                if (error.response) {
                  this.showErrorAlert(`Error deleting user: ${error.response.data.message}`);
                } else {
                  this.showErrorAlert('Error deleting user. Please try again.');
                }
              }
            },
    ...
    async saveSupplierRate() {
      try {
        ...
        if (response.status === 200) {
          this.showSuccessAlert("Supplier rate updated successfully!");
        ...
        if (error.response) {
          ...
          this.showErrorAlert("Error updating supplier rate. Please try again.");
        }
      }
    },
      ...
      try {
        ...
        if (response.status === 200) {
          if (response.data.new_status === 'inactive') {
            ...
            this.showSuccessAlert('Supplier has been set to Inactive');
          } else {
            ...
            this.showSuccessAlert('Supplier has been set to Active');
          }
        ...
        if (error.response) {
          this.showErrorAlert(`Error toggling supplier status: ${error.response.data.message}`);
        } else {
          this.showErrorAlert("Error updating supplier status. Please try again.");
        }
        this.closeStatusConfirmModal();
      }
    },
    async showInactiveSuppliers() {
      try {
        ...
        this.showErrorAlert("Error fetching inactive suppliers. Please try again.");
      }
    },
    ...
    addSocialMedia() {
      ...
      this.showSocialMediaModal = true;
        ...
        this.showErrorAlert('No supplier selected');
      }
    },
    ...
    async submitSocialMedia() {
      try {
        ...
        if (!this.socialMediaForm.platform || !this.socialMediaForm.handle) {
          this.showErrorAlert('Platform and handle are required fields');
          return;
        }
        ...
        if (response.status === 200) {
          this.showSuccessAlert('Social media added successfully!');
          this.closeSocialMediaModal();
          await this.fetchSuppliers(); // Refresh supplier data
        }
      } catch (error) {
        console.error('Error adding social media:', error);
        this.showErrorAlert(error.response?.data?.error || 'Failed to add social media');
      }
    },
    editAdminBtn(index) {
    ...
    async confirmEditAdmin() {
      try {
        ...
        if (!this.selectedAdmin) {
          this.showErrorAlert("No admin selected for editing.");
          return;
        }
        if (!this.selectedAdmin.userid) {
          this.showErrorAlert("Admin ID is missing or invalid.");
          return;
        }
        const response = await axios.put(
        ...
        if (response.status === 200) {
          this.showSuccessAlert("Admin updated successfully!");
          this.closeEditAdminBtn();
        } else {
          this.showErrorAlert("Failed to update admin.");
        }
      } catch (error) {
        ...
        if (error.response) {
          ...
          this.showErrorAlert("Error updating admin. Please try again.");
        }
      }
    },
    async toggleInactiveSupplierStatus(supplier) {
      try {
        ...
        if (response.status === 200) {
          ...
          this.showSuccessAlert('Supplier has been activated successfully');
        ...
        console.error("Error toggling supplier status:", error);
        if (error.response) {
          this.showErrorAlert(`Error activating supplier: ${error.response.data.message}`);
        } else {
          this.showErrorAlert("Error activating supplier. Please try again.");
        }
      }
    },
