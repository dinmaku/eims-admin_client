    ...
    profileImageUrl() {
      ...
      if (this.userProfile && this.userProfile.user_img) {
        if (this.userProfile.user_img.startsWith('http')) {
          ...
          return `${this.apiBaseUrl}/api/admin/profile-image/${this.userProfile.user_img}`;
        }
      }
  ...
  methods: {
    async fetchUserProfile() {
      ...
      try {
        ...
        const response = await axios.get(`${this.apiBaseUrl}/api/admin/profile`, {
          headers: {
    ...
    async handleImageUpload(event) {
      ...
      try {
        ...
        const token = localStorage.getItem('access_token');
        const response = await axios.post(`${this.apiBaseUrl}/api/admin/update-profile-picture`, formData, {
          headers: {
            'Authorization': `Bearer ${token}`,
